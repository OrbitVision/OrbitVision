...
{
    public class SatelliteRefreshService : ISatelliteRefreshService
    {
        private readonly HttpClient _httpClient;
        private readonly AppDbContext _db;
        public SatelliteRefreshService(HttpClient httpClient, AppDbContext db)
        {
            _db = db;
            _httpClient = httpClient;
        }

        public async Task RefreshSatelliteDataAsync()
        {
            try
            {
            ...
            {
                throw new Exception($"Błąd podczas synchronizacji: {ex.Message}");
